...
uint16_t Adafruit_FONA::TCPread(uint8_t *buff, uint16_t len) {
  ...
  if (! parseReply(F("+CIPRXGET: 2,"), &avail, ',', 0)) return false;
	...
	uint16_t read = 0;
	while(avail - read > 0){
		uint16_t newRead = readRaw(avail - read);
		DEBUG_PRINTLN(newRead);
		memcpy(buff + read, replybuffer, newRead + 1); //add 1 extra char to end -> terminator
		DEBUG_PRINTLN((char*)buff);
		read += newRead;
		break;
	}

	#ifdef ADAFRUIT_FONA_DEBUG
	DEBUG_PRINT (read); DEBUG_PRINTLN(F(" bytes read"));
